...
void moloch_config_load()
{
    ...
    config.threatLibDir     = moloch_config_str_list(keyfile, "threatLibDir", "/data/dps/threatlib");
    ...
    config.geoLite2ASN      = moloch_config_str(keyfile, "geoLite2ASN", "/data/dps/etc/GeoLite2-ASN.mmdb");
    config.geoLite2Country  = moloch_config_str(keyfile, "geoLite2Country", "/data/dps/etc/GeoLite2-Country.mmdb");
    ...
    config.parsersDir       = moloch_config_str_list(keyfile, "parsersDir", " /data/dps/parsers ; ./parsers ");
	...
	config.enableThreatCheck = moloch_config_boolean(keyfile, "enableThreatCheck", FALSE);
...
void moloch_config_init()
{
    ...
    if (!config.threatLibDir)
    {
        printf("Must set a threatLibDir to store threat lib files\n");
        exit(1);
    }
...
void moloch_config_exit()
{
    ...
    if (config.threatLibDir)
        g_strfreev(config.threatLibDir);
