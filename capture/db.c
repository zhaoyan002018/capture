...
void moloch_db_save_session(MolochSession_t *session, int final)
{
    ...
    if (IN6_IS_ADDR_V4MAPPED(&session->addr1)) {
        ...
        //威胁匹配ipv4
		//check_ipv4_threat(ip, (uint64_t)session->lastPacket.tv_sec, ipsrc, dbInfo[thread].prefix);
        ...
        //威胁匹配ipv6
		//check_ipv6_threat(session->addr1, (uint64_t)session->lastPacket.tv_sec, ipsrc, dbInfo[thread].prefix);
    }
    BSB_EXPORT_sprintf(jbsb,
                      "\"timestamp\":%" PRIu64 ","
                      "\"srcIp\":\"%s\","
...
char *moloch_db_create_file_full(time_t firstPacket, const char *name, uint64_t size, int locked, uint32_t *id, ...)
{
    ...
    char              *json = moloch_http_get_buffer(MOLOCH_HTTP_BUFFER_SIZE);
    BSB                jbsb;
    uint64_t     fp = firstPacket;
/*add by wj
离线导入的文件时间戳为修改为当前时间戳，原来的时间戳为pcap文件中第一个报文的时间戳，但是在多线程下，第一个保存的会话未必是pcap文件最早的*/
	if (config.pcapReadFiles || config.pcapDir)
	{
		fp = time(NULL);
	}
...
LOCAL void moloch_db_check()
{
    ...
    version = moloch_js0n_get(meta, meta_len, "dpsDbVersion", &version_len);
