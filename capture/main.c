...
{
    { "config",    'c',                    0, G_OPTION_ARG_FILENAME,       &config.configFile,    "Config file name, default '/data/dps/etc/config.ini'", NULL },
...
void moloch_interface_init()
{
	const  size_t BUFF_SIZE = 100;
	char buff[BUFF_SIZE];
	const char* para[] = { "rx", "tx", "sg", "tso", "ufo", "gso", "gro", "lro" };
	int i;
	for (i = 0; i < MAX_INTERFACES && config.interface[i]; i++) {
		memset(buff, 0, BUFF_SIZE);
		snprintf(buff, BUFF_SIZE - 1, "/sbin/ethtool -G %s rx 4096 tx 4096", config.interface[i]);
		system(buff);

		memset(buff, 0, BUFF_SIZE - 1);
		snprintf(buff, BUFF_SIZE - 1, "/usr/sbin/ifconfig %s promisc", config.interface[i]);
		system(buff);
		int len = sizeof(para)/sizeof(char*);
		for (int j = 0; j < len; j++)
		{
			memset(buff, 0, BUFF_SIZE);
			snprintf(buff, BUFF_SIZE - 1, "/sbin/ethtool -K %s %s off ", config.interface[i], para[j]);
			system(buff);
		}
	}
	return 0;
