...
static void scrunch_sf(unsigned char *, unsigned long *);
static void unscrun_sf(unsigned long *, unsigned char *);
static void desfunc_sf(unsigned long *, unsigned long *);
static void cookey_sf(unsigned long *);
...
void deskey_sf(	/* Thanks to James Gillogly & Phil Karn! */
unsigned char *key,
short edf)
{
...
unsigned char *inblock, unsigned char *outblock)
{
	...
	scrunch_sf(inblock, work);
	desfunc_sf(work, KnL);
	unscrun_sf(work, outblock);
	return;
	}

static void scrunch_sf(
...
void des2key_sf(	/* stomps on Kn3 too */
...
short mode)
{
	...
	deskey_sf(&hexkey[8], revmod);
	cpkey_sf(KnR);
	deskey_sf(hexkey, mode);
	cpkey_sf(Kn3);					/* Kn3 = KnL */
	return;
	}

void Ddes_sf(
unsigned char *from,unsigned char *into)		/* unsigned char[8] */
{
	unsigned long work[2];

	scrunch_sf(from, work);
	desfunc_sf(work, KnL);
	desfunc_sf(work, KnR);
	desfunc_sf(work, Kn3);
	unscrun_sf(work, into);
	return;
	}
...
unsigned char *into)			/* unsigned char[16] */
{
	...
	right = bufR;
	l1 = &leftt[1];
	scrunch_sf(from, leftt);
	scrunch_sf(&from[8], right);
	desfunc_sf(leftt, KnL);
	desfunc_sf(right, KnL);
	swap = *l1;
	*l1 = *right;
	*right = swap;
	desfunc_sf(leftt, KnR);
	desfunc_sf(right, KnR);
	swap = *l1;
	*l1 = *right;
	*right = swap;
	desfunc_sf(leftt, Kn3);
	desfunc_sf(right, Kn3);
	unscrun_sf(leftt, into);
	unscrun_sf(right, &into[8]);
	return;
	}
...
void use2key_sf(			/* stomps on Kn3 too */
...
short mode)
{
	...
	deskey_sf(&hexkey[8], revmod);
	cpkey_sf(KnR);
	deskey_sf(third, mode);
	cpkey_sf(Kn3);
	deskey_sf(first, mode);
	return;
	}
...
void D3des_sf(	/* amateur theatrics */
...
unsigned char *into)			/* unsigned char[24] */
{
	...
	scrunch_sf(from, leftt);
	scrunch_sf(&from[8], middl);
	scrunch_sf(&from[16], right);
	desfunc_sf(leftt, KnL);
	desfunc_sf(middl, KnL);
	desfunc_sf(right, KnL);
	swap = leftt[1];
	leftt[1] = middl[0];
	middl[0] = swap;
	...
	right[0] = swap;
	desfunc_sf(leftt, Kn3);
	desfunc_sf(middl, Kn3);
	desfunc_sf(right, Kn3);
	unscrun_sf(leftt, into);
	unscrun_sf(middl, &into[8]);
	unscrun_sf(right, &into[16]);
	return;
	}
