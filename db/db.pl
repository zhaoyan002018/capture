...
sub showHelp($)
{
    ...
    print "  init [<opts>]                - Clear ALL elasticsearch DPS data and create schema\n";
    ...
    print "  upgrade [<opts>]             - Upgrade DPS's schema in elasticsearch from previous versions\n";
    print "    --shards <shards>          - Number of shards for sessions, default number of nodes\n";
    print "    --replicas <num>           - Number of replicas for sessions, default 0\n";
    print "    --shardsPerNode <shards>   - Number of shards per node or use \"null\" to let ES decide, default shards*replicas/nodes\n";
    ...
    my $mapping = '
{
  "field": {
    ...
    esPost("/${PREFIX}fields_v2/field/rootId", '{
      "friendlyName": "根id",
      "group": "general",
      "help": "DPS ID of the first session in a multi session stream",
      "type": "termfield",
      "dbField": "ro",
      "dbField2": "rootId"
    }');
    esPost("/${PREFIX}fields_v2/field/node", '{
      "friendlyName": "DPS 节点",
      "group": "general",
      "help": "DPS node name the session was recorded on",
      "type": "termfield",
      "dbField": "no",
      "dbField2": "node"
    }');
    esPost("/${PREFIX}fields_v2/field/file", '{
      "friendlyName": "文件名",
      "group": "general",
      "help": "DPS offline pcap filename",
      "type": "fileand",
      "dbField": "fileand",
      "dbField2": "fileand"
    }');
    esPost("/${PREFIX}fields_v2/field/payload8.src.hex", '{
      "friendlyName": "源载荷/Hex",
      "group": "general",
...
my ($loud) = @_;
    ...
    if (defined $version &&
        ...
        exists $version->{"${PREFIX}sessions2_template"}->{mappings}->{session}->{_meta}->{dpsDbVersion}
    ) {
        $main::versionNumber = $version->{"${PREFIX}sessions2_template"}->{mappings}->{session}->{_meta}->{dpsDbVersion};
        return;
    }
    ...
    if (!defined $found) {
        logmsg "This is a fresh DPS install\n" if ($loud);
        $main::versionNumber = -1;
        if ($loud && $ARGV[1] !~ "init") {
            die "Looks like DPS wasn't installed, must do init"
        }
    } elsif ($found == 0) {
...
sub dbCheck {
        ...
        $main::esVersion >= 70000)
    {
        logmsg("Currently using Elasticsearch version ", $esversion->{version}->{number}, " which isn't supported\n",
              ...
              "\n",
              "Instructions:www.ruifosang.com",
...
my $nodes = esGet("/_nodes");
$main::numberOfNodes = dataNodes($nodes->{nodes});
logmsg "It is STRONGLY recommended that you stop ALL DPS captures and viewers before proceeding.  Use 'db.pl ${main::elasticsearch} backup' to backup db first.\n\n";
...
if ($ARGV[1] =~ /^(init|wipe|clean)/) {
    ...
    if ($ARGV[1] eq "init" && $main::versionNumber >= 0) {
        logmsg "It appears this elastic search cluster already has DPS installed (version $main::versionNumber), this will delete ALL data in elastic search! (It does not delete the pcap files on disk.)\n\n";
...
# For really old versions don't support upgradenoprompt
    if ($main::versionNumber < 50) {
        logmsg "Can not upgrade directly, please upgrade to DPS 1.0 or 1.1 first. (Db version $main::VersionNumber)\n\n";
        exit 1;
    }
