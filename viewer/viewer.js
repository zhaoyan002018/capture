...
var Config         = require('./config.js'),
    ...
    path           = require('path'),
    rpc = require('./thrift_util/rpc'),
    rmpcap_router = require('./myroutes/rmfiles').router,
    time_router = require('./myroutes/time').router,
    net_router = require('./myroutes/netconf').router,
    login_router = require('./login/login').router,
    logAction = require('./logrecord/history').logMiddle,
    vue_route = require('./vue_router').router;
...
app.locals.dpsVersion =  dpsVersion.version;
...
app.use('/report',express.static(__dirname + "/report",{ maxAge: 600 * 1000}));
...
app.use('/',login_router,(req,res,next)=>{
    if (req.url == '/login'){
        let source = req.session.user;
        userCleanup(source);
        return res.json({"success":true,message: '登陆成功'});
    }
    else{
        next()
    }
...
app.get('/loadSession*',(req,res,next)=>{
    req.query.token = req.query.token || "";
    if ( req.query.token === ""){
        return res.redirect('/login');
    }
    Db.getUserCache("admin",function (err,suser) {
        if (!err && suser && suser.found) {
            userCleanup(suser._source);
            req.query.token = req.query.token || "";
            req.query.token = req.query.token.toLowerCase()
            let myDate = new Date()
            let datestr = myDate.toLocaleDateString().split('-').join('/');
            let token = Config.md5(datestr + "admin" + suser._source.passStore)
            if ( token == req.query.token){
                req.session.user = suser._source;
                const src= req.query.srcIp || "";
                const srcport = req.query.srcPort || "";
                const dst= req.query.dstIp || "";
                const dstport = req.query.dstPort || "";
                let sessionStartTime = req.query.sessionStartTime || "";
                let timestamp = req.query.timestamp || "";
                if (!src || !srcport || !dst || !dstport || !timestamp){
                    return res.redirect("/sessions");
                }
                var sessionUpTime,sessionDownTime
                sessionStartTime = parseInt(sessionStartTime);
                if ( sessionStartTime && sessionStartTime > 1000){
                    sessionUpTime = new Date(sessionStartTime * 1000 - 60*1000)
                    sessionUpTime = Date.parse(sessionUpTime )/1000 + ''
                    sessionDownTime = new Date(sessionStartTime * 1000 + 60*1000)
                    sessionDownTime  = Date.parse(sessionDownTime  )/1000 + ''
                }
                else{
                    timestamp = parseInt(timestamp);
                    if (timestamp && timestamp > 1000 ){
                        sessionUpTime = new Date(timestamp * 1000 - 10*60*1000)
                        sessionUpTime = Date.parse(sessionUpTime )/1000 + ''
                        sessionDownTime = new Date(timestamp * 1000 + 10*60*1000)
                        sessionDownTime  = Date.parse(sessionDownTime )/1000 + ''
                    }
                    else{
                        return res.molochError(200, "非法的时间戳");
                    }
                }
                ...
                let params = {
                    graphType:'lpHisto',
                    seriesType:'bars',
                    expression: `ip.src == ${src} && port.src == ${srcport} && ip.dst == ${dst} && port.dst == ${dstport}`,
                    stopTime:sessionDownTime,
                    startTime:sessionUpTime,
                    bounding:sessionStartTime?'first':'last'
                };
              ...
              return res.molochError(200,"token非法");
            }
...
if (Config.get("passwordSecret")) {
  ...
  app.use(function(req, res, next) {
    ...
    if (req.url === "/_ns_/nstest.html") {
      ...
      //token auth or session auth
      req.session.user = req.session.user || undefined;
      if (req.session.user != undefined){
          req.user = req.session.user;
          return next();
      }
      else {
          req.user = {userId: "anonymous", enabled: true, createEnabled: false, webEnabled: true, headerAuthEnabled: false, emailSearch: false, removeEnabled: false, packetSearch: false, settings: {}};
    ...
    if (req.url.match(/^.*\.(js|css|png)$/)) {
        return next();
    }

    // S2S Auth
    if (req.headers['x-dps-auth']) {
      var obj = Config.auth2obj(req.headers['x-dps-auth']);
      ...
      Db.getUserCache(obj.user, function(err, suser) {
            if (err) {return res.send("ERROR - x-dps getUser - user: " + obj.user + " err:" + err);}
...
var pcapdownload = function(){
    ...
    var middle = express.static(Config.get("pcapDir"),options)
    return middle
}
app.use('/download',pcapdownload())
function loadFields() {
...
function loadPlugins() {
  ...
  var dirs = Config.get("pluginsDir", "/data/dps/plugins").split(";");
...
function errorString(err, result) {
  ...
  if (str.match("IndexMissingException")) {
    return "dps's Elasticsearch database has no matching session indices for timeframe selected";
  } else {
...
function createSessionDetail() {
  ...
  dirs = dirs.concat(Config.get("pluginsDir", "/data/dps/plugins").split(';'));
  dirs = dirs.concat(Config.get("parsersDir", "/data/dps/parsers").split(';'));
...
function addAuth(info, user, node, secret) {
    ...
    info.headers['x-dps-auth'] = Config.obj2auth({date: Date.now(),
                                                     user: user.userId,
                                                     node: node,
                                                     path: info.path
                                                    }, secret);
...
function checkCookieToken(req, res, next) {
  if (!req.headers['x-dps-cookie']) {
    return res.molochError(500, 'Missing token');
  }

  req.token = Config.auth2obj(req.headers['x-dps-cookie']);
...
function checkWebEnabled(req, res, next) {
  if (!req.user.webEnabled) {
    return res.send("DPS Permision Denied");
  }
...
function recordResponseTime (req, res, next) {
  onHeaders(res, () => {
    ...
    ms = Math.ceil(ms);
    res.setHeader('X-dps-Response-Time', ms);
  });
...
app.get('/dpsclusters', function(req, res) {
...
app.get('/user/current', function(req, res) {
 ...
 console.log('/user/current', clone);
...
app.post('/user/settings/update', [checkCookieToken, logAction(), postSettingUser], function(req, res) {
  ...
  Db.setUser(req.settingUser.userId, req.settingUser, function(err, info) {
    ...
    console.log('/user/settings/update', req.body);
    return res.send(JSON.stringify({
      success : true,
...
function buildSessionQuery (req, buildCb) {
  ...
  if (req.query.facets) {
    query.aggregations = {};
    if (req.query.dataTerm === 'ip') {
      query.aggregations = {
          sameIp: { terms: { field: 'srcIp', size: 1000 }, aggregations: { sameDstIp: { terms: { field: 'dstIp' } } } }
      };
      query.aggregations.sameIp.aggregations.sameDstIp.aggregations = {
          srcBytes:{ sum:{ field: 'srcBytes' } },
          dstBytes:{ sum:{ field: 'dstBytes' } },
          srcPackets:{ sum:{ field: 'srcPackets' } },
          dstPackets:{ sum:{ field: 'dstPackets' } }
      };
  } else if (req.query.dataTerm === 'proto') {
          query.aggregations = {
          sameIp: {
              terms: { field: 'srcIp', size: 1000 },
              aggregations: {
                  sameDstIp: {
                      terms: { field: 'dstIp' },
                      aggregations: {
                          sameProto: {
                              terms: { field: 'ipProtocol'}
                          }
                      }
                  }
              }
          ...
          query.aggregations.sameIp.aggregations.sameDstIp.aggregations.sameProto.aggregations = {
          srcBytes:{ sum:{ field: 'srcBytes' } },
          dstBytes:{ sum:{ field: 'dstBytes' } },
          srcPackets:{ sum:{ field: 'srcPackets' } },
          dstPackets:{ sum:{ field: 'dstPackets' } }
        ...
        query.aggregations.sameIp.aggregations.sameDstIp.aggregations.sameSrcIp.aggregations.sameDstPort.aggregations = {
            srcBytes:{ sum:{ field: 'srcBytes' } },
            dstBytes:{ sum:{ field: 'dstBytes' } },
            srcPackets:{ sum:{ field: 'srcPackets' } },
            dstPackets:{ sum:{ field: 'dstPackets' } }
...
app.get('/file/list', logAction('files'), recordResponseTime, function(req, res) {
  ...
  .then(([files, total]) => {
    ...
    for (let i = 0, ilen = files.hits.hits.length; i < ilen; i++) {
      ...
      fields.realpath = fields.name
      fields.name.match(reg) && ( fields.name = fields.name.match(reg)[0] );
      fields.fullname = path.basename(fields.realpath);
      fields.id = files.hits.hits[i]._id;
      results.results.push(fields);
    }
    async.map(results.results,function(item,cb){
                                  fs.stat(item.realpath, function (err, stats) {
                                      var filesize = 0;
                                      if (err || !stats) {
                                          filesize = -1;
                                      } else {
...
function dataMerge (aggregations) {
    let data = [];
    aggregations.sameIp.buckets.forEach(function (item) {
        let field = {};
        field.srcIp = item.key;
        item.sameDstIp.buckets.forEach(function (dst) {
            field.dstIp = dst.key;
            if (dst.sameProto) {
                dst.sameProto.buckets.forEach(function (proto) {
                    field.ipProtocol = proto.key;
                    field.count = proto.doc_count;
                    field.dstPackets = proto.dstPackets.value;
                    field.srcPackets = proto.srcPackets.value;
                    field.dstBytes = proto.dstBytes.value;
                    field.srcBytes = proto.srcBytes.value;
                    data.push(field);
                });
            } else if (dst.sameSrcPort) {
                // field.count = dst.sameSrcPort.
                dst.sameSrcPort.buckets.forEach(function (srcPort) {
                    field.srcPort = srcPort.key;
                    srcPort.sameDstPort.buckets.forEach(function (dstPort) {
                        field.dstPort = dstPort.key;
                        field.count = dstPort.doc_count;
                        field.dstPackets = dstPort.dstPackets.value;
                        field.srcPackets = dstPort.srcPackets.value;
                        field.dstBytes = dstPort.dstBytes.value;
                        field.srcBytes = dstPort.srcBytes.value;
                        data.push(field);
                    });
                });
            } else {
                field.count = dst.doc_count;
                field.dstPackets = dst.dstPackets.value;
                field.srcPackets = dst.srcPackets.value;
                field.dstBytes = dst.dstBytes.value;
                field.srcBytes = dst.srcBytes.value;
                data.push(field);
            }
        });
    });
...
app.get('/sessions.json', logAction('sessions'), recordResponseTime, noCacheJson, function (req, res) {
  ...
  buildSessionQuery(req, function (bsqErr, query, indices) {
    ...
    if (query.aggregations && query.aggregations.sameIp) {
        // polymerization.data =
    }
...
app.get('/dataStatistics', logAction('sessions'), recordResponseTime, noCacheJson, function (req, res) {
    var graph = {};
    var map = {};
    var MergeData = [];
    buildSessionQuery(req, function (bsqErr, query, indices) {
        ...
        Promise.all([Db.searchPrimary(indices, 'session', query),
            ...
            graph = graphMerge(req, query, sessions.aggregations);
            map = mapMerge(sessions.aggregations);
            MergeData = dataMerge(sessions.aggregations);
            var r = {
                recordsTotal: MergeData.length,
                recordsFiltered: MergeData.length,
                graph: graph,
                health: health,
                map: map,
                data: MergeData
...
app.get('/spiview.json', logAction('spiview'), recordResponseTime, noCacheJson, function(req, res) {
  ...
  if (req.query.date === '-1' && spiDataMaxIndices !== -1) {
    return res.send({spi: {}, bsqErr: "请缩小搜索时间范围，考虑到速度，spi视图不允许时间为全部"});
  }
...
function writePcapNg(res, id, options, doneCb) {
  ...
  function(err, session) {
    ...
    session.version = dpsVersion.version;
...
app.post('/upload', multer({dest:'/data/dps/raw'}).single('file'), function (req, res) {
  if (req.file) {
    res.send({
      success: true,
...
app.post('/import', function (req, res) {
  ...
  var cmd = Config.get('uploadCommand')
      ...
      .replace('{TMPFILE}', `/data/dps/raw/${req.body.filename}`)
  ...
  exec(cmd, function (error, stdout, stderr) {
    ...
    res.write(stdout);
    res.end('</pre>');
    // fs.unlink(`/data/dps/raw/${req.body.filename}`);
  });
});
