<template>
  <div>
    <div v-if="compatibleBrowser">
      ...
      <moloch-sidebar></moloch-sidebar>
      <moloch-body></moloch-body>
...
import { MolochBody, MolochFooter, MolochNavbar, MolochSidebar } from './components';
import MolochKeyboardShortcuts from '@/components/KeyboardShortcuts/KeyboardShortcuts';
// import MolochWelcomeMessage from './components/WelcomeMessage';
import MolochUpgradeBrowser from './components/UpgradeBrowser';
export default {
  name: 'Layout',
  components: {
    MolochSidebar,
  ...
  mounted: function () {
    ...
    window.addEventListener('keyup', (event) => {
      ...
      for (let i in this.shiftKey) {
        if (event.keyCode === this.shiftKey[i].keyCode) this.routeTo(this.shiftKey[i].path);
      }
...
/* wrapping */
.no-wrap { white-space: nowrap; vertical-align: top;}
