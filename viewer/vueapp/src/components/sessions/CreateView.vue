  ...
  <div class="row"
    ...
    <div class="col-md-3">
      <div class="input-group input-group-sm">
        ...
        <input v-model="name"
          ...
          maxlength="20"
          class="form-control"
          :placeholder="$t('sessions.cvViewNameTip')"
        />
      </div>
    </div> <!-- /view name input -->
    ...
    <div class="col-md-5 pl-0"
      ...
      <div class="input-group input-group-sm">
        <div class="input-group-prepend">
          <span class="input-group-text">
            {{$t('sessions.cvExpression')}}
          </span>
        </div>
        <input v-model="viewExpression"
          type="text"
          class="form-control"
          :placeholder="$t('sessions.cvExpressionTip')"
        />
...
export default {
  ...
  methods: {
    ...
    createView: function () {
      if (!this.name) {
        this.error = this.$t('sessions.cvErr1');
        return;
      }

      if (!this.viewExpression) {
        this.error = this.$t('sessions.cvErr2');
        return;
      }
