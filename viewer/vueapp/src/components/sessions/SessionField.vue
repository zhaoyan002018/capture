    ...
    <span v-if="!field.children && parsed !== undefined">
      <span v-for="pd of parsed"
        ...
        <span v-if="!time"
          ...
          <div v-if="isOpen"
            ...
            <b-dropdown-item
              @click.prevent.stop="fieldClick(expr, pd.queryVal, '==', '||')"
              :title="'|| ' + expr + ' == ' + pd.value">
              <strong>{{$t('sessions.sfOr')}}</strong>
              {{ pd.value }}
            </b-dropdown-item>
            <b-dropdown-item
              @click.prevent.stop="fieldClick(expr, pd.queryVal, '!=', '||')"
              :title="'|| ' + expr + ' != ' + pd.value">
              <strong>{{$t('sessions.sfOrNot')}}</strong>
              {{ pd.value }}
            </b-dropdown-item>
            <span v-if="session && field.portField && session[field.portField] !== undefined">
              <b-dropdown-item
                ...
                :title="'&& ' + expr + ' == ' + pd.value">
                <strong>{{$t('sessions.sfAnd')}}</strong>
                {{ pd.value }}:{{ session[field.portField] }}
              </b-dropdown-item>
              <b-dropdown-item
                @click.prevent.stop="fieldClick(expr, pd.queryVal + ':' + session[field.portField], '!=', '&&')"
                :title="'&& ' + expr + ' != ' + pd.value">
                <strong>{{$t('sessions.sfAndNot')}}</strong>
                {{ pd.value }}:{{ session[field.portField] }}
              </b-dropdown-item>
              <b-dropdown-item
                @click.prevent.stop="fieldClick(expr, pd.queryVal + ':' + session[field.portField], '==', '||')"
            ...
            <b-dropdown-item
              ...
              :title="$t('sessions.sfOiswevaddtoTip',[expr,pd.queryVal])">
              <span class="fa fa-folder-open-o"></span>&nbsp;
              {{$t('sessions.sfOpenSessions')}}
            </b-dropdown-item>
            <b-dropdown-item
              @click.prevent.stop="newTabSessions(expr, pd.queryVal, '==')"
              :title="$t('sessions.sfOiswevaddtoTip',[expr,pd.queryVal])">
              <span class="fa fa-external-link-square"></span>&nbsp;
              {{$t('sessions.sfOpenSessionsTab')}}
            </b-dropdown-item>
            <b-dropdown-item
              v-if="expression"
              @click.prevent.stop="newTabSessions(expr, pd.queryVal, '==', true)"
              :title="'使用' + expr + ' == ' + pd.queryVal + '作为基础过滤表达式打开会话标签页'">
        ...
        <span v-else
          ...
          :title="$t('sessions.sfCtApplyTip',[field.friendlyName])"
          v-b-tooltip.hover
