              ...
              <div>
                ...
                <b-dropdown right
                  ...
                  <b-dropdown-item v-if="!column.nodeExcluded"
                    ...
                    {{$t('common.exclude')}} {{$t('common.node')}} {{ column.name }}
                  </b-dropdown-item>
                  <b-dropdown-item v-if="column.nodeExcluded"
                    @click="include('name', column)">
                    {{$t('common.include')}} {{$t('common.node')}} {{ column.name }}
                  </b-dropdown-item>
                  <b-dropdown-item v-if="!column.ipExcluded"
                    @click="exclude('ip', column)">
                    {{$t('common.exclude')}} IP {{ column.ip }}
                  </b-dropdown-item>
                  <b-dropdown-item v-if="column.ipExcluded"
                    @click="include('ip', column)">
                    {{$t('common.include')}} IP {{ column.ip }} {{ column.ip }}
                  </b-dropdown-item>
              ...
              <template v-if="stat.nodes[node]"
                ...
                <span :key="node + '-' + stat.name + '-' + item.shard + '-shard'"
                  ...
                  <span v-if="item.showDetails"
                    ...
                    <div v-if="item.fm">
                      <span>{{$t('common.field')}} {{$t('common.memory')}}:</span>
                      {{ item.fm | humanReadableBytes}}
                    </div>
                    <div v-if="item.sm">
                      <span>{{$t('common.segment')}} {{$t('common.memory')}}:</span>
                      {{ item.sm | humanReadableBytes}}
                    </div>
                    <div>
                      <span>{{$t('common.shard')}} {{$t('common.type')}}:</span>
