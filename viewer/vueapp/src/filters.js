 ...
 */
Vue.filter('humanReadableBytes', (fileSizeInBytes, label = 'B') => {
  fileSizeInBytes = parseInt(fileSizeInBytes);
  let i = 0;
  let byteUnits = ['', 'K', 'M', 'G', 'T', 'P', 'E', 'Z', 'Y'];
  ...
  if (i === 0 || fileSizeInBytes >= 10) {
    return fileSizeInBytes.toFixed(0) + byteUnits[i] + label;
  } else {
    return fileSizeInBytes.toFixed(1) + byteUnits[i] + label;
  }
});
